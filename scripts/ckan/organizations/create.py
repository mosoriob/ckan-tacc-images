...
def create_organization(org_data: Dict, api_key: str, ckan_url: str) -> bool:
    ...
    try:
        ...
        print(f"Error creating organization {org_data.get('name', 'unknown')}: {str(e)}")
        return False

def main():
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Create organizations in CKAN from a JSON file.')
    parser.add_argument('--api-key', required=True, help='Your CKAN API key')
    parser.add_argument('--json-file', required=True, help='Path to JSON file containing organization data')
